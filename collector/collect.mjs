  ...
  for (const source of browserSources) {
    for (const rule of rules) {
      ...
      try {
        ...
        if ([401, 403, 429].includes(code)) throw new Error(`bloqueio HTTP ${code}; coleta interrompida`);
...
const ingest = await fetch(`${baseUrl.replace(/\/$/, "")}/internal/ingest`, { method: "POST", headers, body: JSON.stringify({ offers }) });
